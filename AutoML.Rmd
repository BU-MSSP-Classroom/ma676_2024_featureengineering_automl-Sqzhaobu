...
```{r,echo=TRUE}
numeric_vars = ames_raw_2008 %>% select(where(is.numeric)) 
BiocManager::install("pcaMethods")
```
```{r,echo=TRUE}
pca_numeric<-pcaMethods::pca(numeric_vars,nPcs=20,scale="uv")
...
```
```{r}
#We choose PC1 - PC 14 to fit a new model
pc_scores <- scores(pca_numeric)[, 1:14]
# Combine PC scores with response variable
data <- data.frame(ames_raw_2008$SalePrice, pc_scores)

# Fit linear regression model
model <- lm(ames_raw_2008$SalePrice ~ ., data = data)
summary(model)
```
